...
SA - PRICING DRIVER (WITH RELAXATION CHECK)
-------------------------------------------
...
3. Output: Compares Real vs Relaxed objective and time per iteration.
"""
# ==============================================================================
import os
import sys
...
class PersistentEvaluator:
    def __init__(self, data, is_relaxed=False):
        ...
        # Flow Variables (Always Relaxed for efficiency, w dictates integrality usually)
...
def run_sa_pricing_driven(instance, max_seconds=120):
    print(f"Running SA (Pricing Driver + Relaxed Check) for {instance}")
    
    data = InstanceData(instance)
    
    # 1. Setup TWO Evaluators
    evaluator_mip = PersistentEvaluator(data, is_relaxed=False) # Integer
    evaluator_lp  = PersistentEvaluator(data, is_relaxed=True)  # Continuous (Relaxed)
    ...
    print("\nIter  | Temp       | New Obj         | Rel Obj         | Best Obj        | Eval(s)  | Rel(s)   | Status")
    print("-" * 110)
    ...
    while (time.time() - start_time) < max_seconds:
        ...
        print(f"{total_iter:<5} | {T:<10.2f} | {n_obj:<15,.0f} | {n_obj_rel:<15,.0f} | {best_obj:<15,.0f} | {time_mip:<8.4f} | {time_lp:<8.4f} | {status}")
