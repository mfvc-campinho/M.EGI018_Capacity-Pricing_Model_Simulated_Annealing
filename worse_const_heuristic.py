...
SA - FAST RELAXATION ACCELERATOR (SUBOPTIMAL START VERSION)
--------------------------------
...
# 3. HEURISTICS
# ==============================================================================
def generate_suboptimal_pricing(data):
    """
    MODIFIED: This heuristic is 'smart' but indecisive.
    It calculates the revenue for all options, identifies the Top 3 best prices,
    and randomly picks one of them.
    This creates a good, but not perfect, starting solution (approx 66% suboptimal).
    """
    target_p = {}
    for r in range(data.R):
        for a in range(data.A + 1):
            
            # 1. Evaluate all price options for this specific rental request
            options = []
            ...
            # 3. Pick randomly from the Top 3 (or fewer if less exist)
            k = min(len(options), 3)
            
            # Random selection from the top K
            chosen_idx = random.randint(0, k - 1)
            best_p = options[chosen_idx][0]
...
def perturb_pricing(pricing_in, P_max, mutation_rate):
    ...
    num_changes = int(len(keys) * mutation_rate)
...
# ==============================================================================
# 4. SA MAIN LOOP (STAGNATION RESET 10 + HALF-DROP)
# ==============================================================================
def run_sa_fast_relaxation(instance, max_seconds=120):
    print(f"Running SA (Suboptimal Start - Top 3 Random) for {instance}")
    ...
    # 2. Initial Solution (NOW SUBOPTIMAL)
    curr_price = generate_suboptimal_pricing(data)
    ...
    # --- STRATEGY PARAMETERS ---
    MAX_MUT_RATE = 0.50       # 50%
    MIN_MUT_RATE = 0.05       # 5%
    STAGNATION_LIMIT = 10     # RESET
    ...
    while (time.time() - start_time) < max_seconds:
        ...
        # -----------------------------------------------------
        # 1. CHECK FOR STAGNATION (RESET AT 10)
        # -----------------------------------------------------
        ...
        # 2. Perturb Pricing
        n_price = perturb_pricing(curr_price, data.P, mutation_rate=current_mutation_rate)
        ...
        if accepted:
            ...
            if curr_obj > best_obj_lp:
                # FOUND IMPROVEMENT -> RESETS THE STAGNATION COUNTER
                iter_since_best = 0 
                
                # Calculate percentage of improvement
                if prev_best_obj == 0: improvement_pct = 1.0 
                else: improvement_pct = (curr_obj - prev_best_obj) / abs(prev_best_obj)
                ...
                if improvement_pct >= 1.0:
                    ...
                    target_rate = MIN_MUT_RATE + (improvement_pct * amplitude)
                    
                    # This creates a soft damping effect.
                    current_mutation_rate = (current_mutation_rate + target_rate) / 2
                
                # Limits
                current_mutation_rate = max(MIN_MUT_RATE, min(MAX_MUT_RATE, current_mutation_rate))

        # Logging
        if True: 
            mut_str = f"{current_mutation_rate*100:.1f}%"
            print(f"{total_iter:<5} | {int(T):<8} | {mut_str:<6} | {n_obj:<14,.0f} | {best_obj_lp:<14,.0f} | {note_str}")
    ...
    save_result("BestSolution_SuboptimalStart.xlsx", final_fleet, best_price)

if __name__ == "__main__":
    INSTANCE = r"data\\Inst50.xlsx"
