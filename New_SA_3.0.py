...
# [ ... INSERT CLASS 1: InstanceData HERE ... ]
# [ ... INSERT CLASS 2: PersistentEvaluator HERE ... ]
...
# ==============================================================================
# 4. SA MAIN LOOP (STAGNATION RESET 10 + SATURATION DROP)
# ==============================================================================
def run_sa_fast_relaxation(instance, max_seconds=120):
    ...
    # --- STRATEGY PARAMETERS ---
    MAX_MUT_RATE = 0.50       # 50% (Caos Máximo)
    MIN_MUT_RATE = 0.05       # 5% (Ajuste Fino)
    STAGNATION_LIMIT = 10     # Limite para Reset
    SATURATION_POINT = 3.0    # 300% (Melhoria necessária para atingir mutação máx)
    ...
    while (time.time() - start_time) < max_seconds:
        ...
        if accepted:
            ...
            # SE FOR UM NOVO RECORDE GLOBAL
            if curr_obj > best_obj_lp:
                iter_since_best = 0 # Reseta contador de estagnação
                
                # --- A. CÁLCULO CORRETO DA MELHORIA ---
                # Compara o NOVO valor contra o RECORDE ANTIGO
                if best_obj_lp == 0: 
                    improvement_pct = 1.0 
                else: 
                ...
                # --- B. LÓGICA DE SATURAÇÃO (PROPORCIONAL) ---
                # Quanto maior a melhoria, maior a mutação (Exploração)
                # Mas graduada pelo SATURATION_POINT (300%)
                
                # Ex: Imp 150% (1.5) / Sat 3.0 = Ratio 0.5
                # Ex: Imp 300% (3.0) / Sat 3.0 = Ratio 1.0 (Máximo)
                ratio = improvement_pct / SATURATION_POINT
                
                # Calcula a taxa alvo
                target_rate = MIN_MUT_RATE + (ratio * amplitude)
                
                # --- C. HALF-DROP (Média com a taxa anterior) ---
                current_mutation_rate = (current_mutation_rate + target_rate) / 2
                
                # --- D. CLAMP FINAL (Segurança) ---
                # Garante que não passamos de 50% nem descemos de 5%
